...
class EMS(gym.Env):
    ...
    def step(self, action):
        ...
        return [self.sun_power, self.price, self.storage, self.moles, self.natural_gas_price, self.ammonia_total - self.ammonia_produced], r, self.done, info 
    ...
    def reward(self, P_grid):
        ...
        #print(too_much_penalty)
        #too_less_penalty = -0.01 * over_under_production if self.ammonia_produced < self.ammonia_total else 0

        
        reward += paid_price - price_natural_gas + price_ammonia + self.hydrogen + too_much_penalty
    ...
    def reset(self):
        ...
        return [self.sun_power, self.price, self.storage, self.moles, self.natural_gas_price, self.ammonia_total - self.ammonia_produced] 
