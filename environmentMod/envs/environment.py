...
class EMS(gym.Env):
    def __init__(self, dynamic=True, eval=False, pv_gen=True, wind_gen=True, discrete=False, multiagent=False, n_agents=3):
        ...
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(INPUT_DIMS+1,), dtype=np.float32)
        ...
        self.sun_power = self.sun_power/5

        self.wind_powers = pd.read_csv('data/environmentMod/renewables/data_wind.csv', header=0, delimiter=',').iloc[self.random_day:self.random_day+NUM_TIME_STEPS+1,4]
    ...
    def step(self, action):
        ...
        if self.wind_gen:
            self.wind_generation = self.windkraft_ertrag(self.wind_power) * 5
        ...
        self.power_from_grid = Pel + Cbattery - Dbattery - pv_generation + np.abs(Wcomp*1e-3) - self.wind_generation
 
        self.ammonia_left = self.ammonia_total - self.ammonia_produced
        ...
        writer.add_scalar('Actions/Wind Generation', self.wind_generation * 1e-6, self.time)
        writer.add_scalar('Actions/Wind Speed', self.wind_power, self.time)
        ...
        return [self.sun_power, self.price, self.storage, self.moles, self.natural_gas_price, self.wind_generation], r, self.done, info 
    ...
    def reward(self, P_grid):
        ...
        if P_grid >=0:
            ...
            paid_price = P_grid*self.price * 0.01
            # 0.005
        ...
        too_much_penalty = -0.01 * over_under_production if self.ammonia_produced > self.ammonia_total else 0
    ...
    def reset(self):
        ...
        if self.eval:
            ...
            self.ammonia_total = 10000 * NUM_TIME_STEPS 
        ...
        return [self.sun_power, self.price, self.storage, self.moles, self.natural_gas_price, self.wind_generation] 

    def get_cp(self, windgeschwindigkeit):
        """Gibt den Leistungsbeiwert basierend auf der Windgeschwindigkeit zurück."""
        if windgeschwindigkeit < 4:
            return 0.4
        elif windgeschwindigkeit < 6:
            return 0.4
        elif windgeschwindigkeit < 9:
            return 0.45
        elif windgeschwindigkeit < 12:
            return 0.35
        elif windgeschwindigkeit < 14:
            return 0.25
        elif windgeschwindigkeit < 17:
            return 0.2
        elif windgeschwindigkeit < 40:
            return 0.1
        else:
            return 0
    ...
    def windkraft_ertrag(self, windgeschwindigkeit):
        ...
        A = 12688 # Fläche der Rotorblätter in m^2
        abschaltgeschwindigkeit = 34.0 # Abschaltgeschwindigkeit in m/s
        # Erhalt des Leistungsbeiwerts basierend auf der Windgeschwindigkeit
        Cp = self.get_cp(windgeschwindigkeit)
        
        if windgeschwindigkeit >= abschaltgeschwindigkeit or windgeschwindigkeit < 3: # Berücksichtigung einer Mindestwindgeschwindigkeit
            ertrag = 0
        else:
            ertrag = 0.5 * rho * A * Cp * (windgeschwindigkeit**3)
