...
def run(ckpt, model_type, dynamic, soft, eval, noisy):


    save_dir = 'data/output/' + model_type + '/simulation2'
    
    if not eval:
        # Create save dir   
        new_logger = configure(save_dir, ["stdout", "csv"])

    env = gym.make('environmentAmmonia/GridWorld-v0')
    ...
    if model_type == "PPO":
        ...
        if eval:
            model = PPO.load(f"{save_dir}/model")
        else:
    ...
    with open(os.getcwd() + '/data/output/' + model_type + '/simulation2/' + model_type + '_eval.pkl', 'wb') as f:
    ...
    with open(os.getcwd() + '/data/output/' + model_type + '/simulation2/' + model_type + '_policy_eval.pkl', 'wb') as f:
        pkl.dump(eval_data, f)
