...
class EMS(gym.Env):
    def __init__(self, dynamic=True, eval=False, pv_gen=True, wind_gen=True, discrete=False, multiagent=False, n_agents=3):
        ...
        if self.eval:
            self.random_day = 26305
            
        else:
            self.random_day = np.minimum(0, 26304 - NUM_TIME_STEPS - 24 * random.randint(0,1096))
          

        self.sun_powers = pd.read_csv('data/environmentSofia/renewables/data.csv', header=0, delimiter=',').iloc[self.random_day:self.random_day+NUM_TIME_STEPS+1,1]
        self.sun_power = self.sun_powers[self.random_day]

        self.wind_powers = pd.read_csv('data/environmentSofia/renewables/data.csv', header=0, delimiter=',').iloc[self.random_day:self.random_day+NUM_TIME_STEPS+1,7]
        self.wind_power = self.wind_powers[self.random_day]

        self.prices = pd.read_csv('data/environmentSofia/prices/data.csv', header=0, delimiter=';').iloc[self.random_day:self.random_day+NUM_TIME_STEPS+1,5]
        self.price = self.prices[self.random_day]

        self.dates = pd.read_csv('data/environmentSofia/prices/data.csv', header=0, delimiter=';').iloc[self.random_day:self.random_day+NUM_TIME_STEPS+1,0]
    ...
    def step(self, action):
        ...
        return [self.sun_power, self.price, self.storage, self.moles, self.natural_gas_price], r, self.done, info 
    ...
    def reset(self):
        ...
        self.natural_gas_prices = pd.read_csv('data/environmentSofia/gas/data.csv', header=0, delimiter=';')
