...
def run(ckpt, model_type, dynamic, soft, transfer, noisy, eval):
    ...
    save_dir = 'data/output/' + model_type + '/simulation4/'
    
    if not eval:
        # Create save dir   
        new_logger = configure(save_dir, ["stdout", "csv"])

    env = gym.make('environmentMod/GridWorld-v0')
    ...
    with open(os.getcwd() + '/data/output/' + model_type + '/simulation4/' + model_type + '_eval.pkl', 'wb') as f:
